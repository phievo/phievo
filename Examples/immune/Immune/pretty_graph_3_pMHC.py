...
def produce_KPRU_name(node_KRPU):
    return "$KRPU_{}$".format(node_KRPU.int_id())

def pretty_graph(net,extended=True,layout="hierarchical"):
    """
    ...
    graph = PlotGraph.Graph(layout=layout)
    ...
    for nn in net.dict_types['Node']:
        ...
        if isinstance(nn, TModule):
            if net.graph.successors(nn)[0] == net.graph.predecessors(nn)[0]:
            ...
            if isinstance(nn, TFHill) or isinstance(nn, CorePromoter):
                ...
                if not extended:
                    next
                binding_name = produce_KPRU_name(nn)
                binding_complex = net.graph.predecessors(nn)[0]
