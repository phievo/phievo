   ...
   for(t=tinduction; t<NSTEP; t++){
	if( t < tfall && indexinduction == 0){
        	currdiff1 = fabs(history[trackout[0]][t][k]-history[trackout[0]][tfall-1][k]); 
                if( currdiff1/history[trackout[0]][tfitness][k] > 0.05) indexinduction = t;
	}
	else if(t > tfall && indexinduction > 0 && indexfall == 0){   
        	currdiff2 = fabs(history[trackout[0]][t][k]-history[trackout[0]][NSTEP-1][k]); 
		if( currdiff2/history[trackout[0]][tfall-1][k] < 0.05) indexfall=t;
   	}
    }
