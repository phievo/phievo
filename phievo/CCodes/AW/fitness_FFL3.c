...
void  fitness( double history[][NSTEP][NCELLTOT], int trackout[],int ntry)  {
    ...
    double diff1, diff2,currdiff1,currdiff2;
	...
	nogood(ntry); return;
      ...
      if( diff1 < 0.001 || fabs(diff1/history[trackout[0]][tfitness][k]-1) < 0.1 ){ 
	nogood(ntry); return;
      } 
   ...
   for(t=tinduction; t<NSTEP; t++){
	if( t < tfall){
        	currdiff1 = fabs(history[trackout[0]][t][k]-history[trackout[0]][tfall-1][k]); 
		if( currdiff1/diff1 <0.05 &&(indexinduction==0) ){
	  		indexinduction=t;
		}
		if( currdiff1 > olddiff1 && currdiff1/diff1 > 0.05) indexinduction = 0;
		olddiff1 = currdiff1; 
	}
	else{   
        	currdiff2 = fabs(history[trackout[0]][t][k]-history[trackout[0]][NSTEP-1][k]); 
		if ( currdiff2/diff2 < 0.05  && indexfall == 0){
	  		indexfall=t;
		}
		if( currdiff2 > olddiff2 && currdiff2/diff2 > 0.05) indexfall = 0;  
        	olddiff2 = currdiff2;
   	}
    }
