...
void  fitness( double history[][NSTEP][NCELLTOT], int trackout[],int ntry)  {
    ...
    double trajmax = 0, initpoint, finalpoint, calcfitness = 0;
    int ind = trackout[0];
    for( i = 0 ; i < NSTEP; i++)  
	if( history[ind][i][0] > trajmax) trajmax = history[ind][i][0];
    initpoint = history[ind][0][0]; finalpoint = history[ind][NSTEP-1][0];
	
    calcfitness -=  trajmax;
    calcfitness +=  finalpoint;
    calcfitness +=  initpoint;
    //fprintf(fp,"%f %f %f %f\n",trajmax,initpoint,finalpoint,calcfitness);
    //fclose(fp);
    
    result[ntry][0]=calcfitness;
