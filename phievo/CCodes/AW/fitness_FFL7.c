...
void  fitness( double history[][NSTEP][NCELLTOT], int trackout[],int ntry)  {
	...
	nogood(ntry); return;
      ...
      if( diff1 < 0.001 || diff1/history[trackout[0]][tfitness][k] < 0.1 ){ 
	nogood(ntry); return;
      } 
   ...
   for(t=tinduction; t<NSTEP; t++){
	if( t < tfall && indexinduction == 0){
        	currdiff1 = fabs(history[trackout[0]][t][k]-history[trackout[0]][tfitness][k]); 
                if( currdiff1/diff1 > 0.05) indexinduction = t;
	}
	else if(t > tfall&& indexinduction > 0){   
        	currdiff2 = fabs(history[trackout[0]][t][k]-history[trackout[0]][NSTEP-1][k]); 
		if ( abs(1-currdiff2/diff2) < 0.05  && indexfall == 0){
	  		indexfall=t;
		}
