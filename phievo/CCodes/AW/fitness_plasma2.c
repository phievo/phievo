...
void  fitness( double history[][NSTEP][NCELLTOT], int trackout[],int ntry)  {
    ...
    result[ntry][1] = mut+1.0*rand()/((double) RAND_MAX + 1); //store total mutations with random component
...
void treatment_fitness( double history2[][NSTEP][NCELLTOT], int trackout[]){
  ...
  int l,k;
  double score,nonmon,corr,avgI,avgM,devI,devM, mut0;
  ...
  mut0 = result[0][1];  // total mutuations without input
  
  score = nonmon+corr+mut0;
