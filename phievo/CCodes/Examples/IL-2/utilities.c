...
/*void print_history( int try,int ncell )  {

    int pas, i, j;
    char titre[50];
    FILE *fileptr;
    double myNcell;
    myNcell = (double)N_to_N_cell_(ncell);
    
    sprintf(titre, "Buffer%i_Dose%.2e_N_cell_%.1e", try, N_to_Dose(ncell), myNcell);
    fileptr=fopen(titre, "w");

    for(pas=0; pas<NSTEPTOT; pas++)  {
	fprintf(fileptr,"%i\t", pas);
	fprintf(fileptr,"%.10f", full_time[pas][ncell]);
	for(i=0; i<SIZE; i++)  {
		fprintf(fileptr,"\t%f",full_history[i][pas][ncell]);
	}
	fprintf(fileptr,"\n");
    }
    fprintf(fileptr,"\n");
