...
void  fitness( double history[][NSTEP][NCELLTOT], int trackout[],int ntry)  {
   ...
   double dev;
    for(k=0; k<NCELLTOT; k++)  {
      ...
      for (step=0;step<NSTEP/2;step++)
      	{
      	dev=history[trackout[0]][step][k]-0.1;
      	cumul=cumul+dev*dev;
      	dev=(history[trackout[1]][step][k]-1);
      	cumul=cumul+dev*dev;
      	}
    for (step=NSTEP/2;step<NSTEP;step++)
    	{
    	dev=history[trackout[1]][step][k]-0.1;
      	cumul=cumul+dev*dev;
      	dev=(history[trackout[0]][step][k]-1);
      	cumul=cumul+dev*dev;
