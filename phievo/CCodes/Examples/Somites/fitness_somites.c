...
static double min_history = 0.02;  // add rand value to history to kill info if h->0
...
double threshold_concentration( double thresh, double out ) {
  /* compute prob that output variable assigned to 1 */
  return out*out/(thresh*thresh + out*out);
}
...
void output_prob( double prob[][MAXSTATES] ) {
  /* take output[ntries] array and compute compute un-normed prob.  
     ...
     Define p[][0] = sum over all outputs of p0
     prob[][i] = p1 for output i = 1,2,3..  
  ...
  */
  ...
  for(n=0; n<NCELLTOT; n++ ) {
    ...
    if (track_zero==0)//if no output is expressed
      {
	for(i=0; i<NOUTPUT; i++) {
	  prob[n][i]=1.0;
	}
  ...
  for (ncell=0;ncell<NCELLTOT;ncell++)
    {
      ...
      if (output_ncell[ncell][0]<0.1)
	test_down=1;
      if ((output_ncell[ncell][0]>0.5)&&(test_down==1))
				       {nup+=1;
					 test_down=0;
					   }
  ...
  for (ncell=0;ncell<NCELLTOT;ncell++)
    {
      ...
      if (output_ncell[ncell][0]>0.5)
	test_down=1;
      if ((output_ncell[ncell][0]<0.1)&&(test_down==1))
				       {nup+=1;
					 test_down=0;
					   }
