...
def compute_KPR_Binding(net):
    ...
    if ('KPR_Binding' in net.list_types):
        for index in net.list_types['KPR_Binding']:
            ...
            func=func+deriv2_pMHC.compute_leap([R.id,L.id],[C.id],arate)
            # self term
            number_species = len(net.list_types['Species'])
            arate="%.10f*"%index.association+"s[%d]"%number_species+"*"+R.id;
            func = func+deriv2_pMHC.compute_leap([R.id,"s[%d]"%number_species],["s[%d]"%(number_species+1)],arate);
    return func
deriv2_pMHC.compute_KPR_Binding=compute_KPR_Binding
