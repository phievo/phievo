...
import copy
import phievo.Networks.Immune.deriv2_pMHC as deriv2_pMHC
...
def compute_KPR_Unbinding(net):
    ...
    if ('KPR_Unbinding' in net.list_types):
        for index in net.list_types['KPR_Unbinding']:
            ...
            # agonist term
            drate="1.0/tau*"+C.id
            func=func+deriv2_pMHC.compute_leap([C.id],[L.id,R.id],drate)
            # self term
            n = C.n_phospho
            number_species = len(net.list_types['Species'])
            drate="1.0/TAU_SELF*s[%d]"%(number_species+n+1)
            func=func+deriv2_pMHC.compute_leap(["s[%d]"%(number_species+n+1)],["s[%d]"%number_species,R.id],drate)
    return func


deriv2_pMHC.compute_KPR_Unbinding=compute_KPR_Unbinding
